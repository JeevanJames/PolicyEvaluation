...
public class PolicyEvaluator<TState>
    where TState : class
{
    private readonly PolicyEvaluatorFunc<TState> _evaluator;
    private readonly PolicyEvaluatorOptions _options;

    public PolicyEvaluator(PolicyEvaluatorFunc<TState> evaluator,
    ...
    /// </exception>
    public IExpressionEvaluationOutcome EvaluateExpression(string expression, TState? state = null)
    {
        ...
        Expression<TState> expr = CreateExpression(tokens);
    ...
    private Expression<TState> CreateExpression(PositionalToken[] tokens)
    {
        ...
        foreach (PositionalToken token in tokens)
        {
            switch (token)
            {
                ...
                case CloseParenthesisToken:
                    nested = false;
                    outerTokens.Add(new Expression<TState>(nestedTokens.ToList(), _options));
                    nestedTokens.Clear();
                    continue;
            }
        ...
        return new Expression<TState>(outerTokens, _options);
    }
